...
function ImportDialogPortal({ children }: { children: ReactNode }) {
  useEffect(() => {
  ...
  // Dashboard containers can have transforms or overflow, so render at document level.
        ...
        <ImportDialogPortal>
          <div className='fixed inset-0 z-50 flex h-dvh w-screen items-center justify-center overflow-hidden p-4'>
            ...
            <section
              role='dialog'
              aria-modal='true'
              aria-label='Import Books From Excel'
              className='relative z-10 flex max-h-[calc(100dvh-2rem)] w-full max-w-3xl min-w-0 flex-col overflow-hidden rounded border border-slate-700 bg-[#070F1D] shadow-2xl'
            >
              <header className='flex shrink-0 items-start justify-between border-b border-slate-800 px-6 py-5'>
                <div>
              ...
              <footer className='flex shrink-0 justify-end gap-3 border-t border-slate-800 px-6 py-4'>
                <button
                  type='button'
                  onClick={close}
                  disabled={isImporting}
