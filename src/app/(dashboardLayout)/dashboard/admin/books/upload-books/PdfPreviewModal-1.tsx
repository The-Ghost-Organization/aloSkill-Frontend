...
export default function PdfPreviewModal({ url, fileName, onClose }: PdfPreviewModalProps) {
  ...
  useEffect(() => {
    ...
    async function fetchPdfBlob() {
      try {
        setLoadError(null);
        // If it is already a local blob, use directly
        ...
        if (active) setLoadError(err.message || "Failed to load PDF");
      }
    }
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
      ...
      <div className='relative z-10 w-full max-w-4xl h-[85vh] bg-[#1a1d24] border border-white/10 rounded shadow-xl flex flex-col overflow-hidden'>
        ...
        <div className='flex items-center justify-between px-6 py-4 border-b border-white/5 bg-[#0e1117] shrink-0'>
          ...
          <button onClick={onClose} className='p-2 rounded text-gray-400 hover:text-white hover:bg-white/10'>
            <X size={20} />
          </button>
        </div>
        ...
        <div className='flex-1 overflow-y-auto bg-[#2b2f36] p-4 flex justify-center'>
          {loadError ? (
            <div className='text-red-400 text-sm m-auto'>{loadError}</div>
          ) : !pdfSource ? (
            <div className='text-white text-sm m-auto'>Loading PDF...</div>
          ) : (
            <Document
              file={pdfSource}
              onLoadSuccess={onDocumentLoadSuccess}
              loading={<div className='text-white text-sm'>Rendering document...</div>}
              error={<div className='text-red-400 text-sm'>Failed to render PDF.</div>}
              className='flex flex-col gap-4'
            >
              {Array.from(new Array(numPages), (_, index) => (
                <Page
