...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async jwt({ token, user }) {
      ...
      const isAdmin = Array.isArray(token["role"])
        ? token["role"].includes("ADMIN")
        : token["role"] === "ADMIN";

      const loginTime = (token["loginTime"] as number) || 0;
      const TWENTY_FOUR_HOURS_MS = 24 * 60 * 60 * 1000;

      if (isAdmin && now - loginTime > TWENTY_FOUR_HOURS_MS) {
        return { ...token, error: "AdminSessionExpired", accessToken: null };
      }
