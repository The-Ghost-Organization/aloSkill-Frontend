...
function VerifyPayment() {
  ...
  const isCashOnDelivery = searchParams.get("payment") === "cod";

  const verifyPayment = useCallback(async () => {
    setState({ status: "verifying" });

    if (isCashOnDelivery && codOrderId) {
      setState({
        status: "success",
    ...
    try {
      const verifyPaymentInfo = await apiClient.get<{ orderStatus: string }>(
        `/order/verify-payment?invoice_id=${invoiceId}`
      );
      ...
      setState({
        status: "failed",
        message:
          "Something went wrong while confirming your payment. Please don't try paying again before checking with support.",
      });
    }
  }, [codOrderId, invoiceId, isCashOnDelivery]);
          ...
          {state.status === "success" && (
            <div key='success'>
              ...
              <h1 className='mb-3 font-(family-name:--font-display) text-3xl font-bold leading-tight opacity-0 motion-safe:animate-[riseIn_0.6s_ease-out_0.3s_forwards] motion-reduce:opacity-100 sm:text-4xl'>
                {isCashOnDelivery ? "Your COD order is placed 🎉" : "You&apos;re all set 🎉"}
              </h1>
              ...
              <div className='mb-9 flex flex-wrap items-center justify-center gap-2 opacity-0 motion-safe:animate-[riseIn_0.6s_ease-out_0.5s_forwards] motion-reduce:opacity-100'>
                ...
                <span className='rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-white/80'>
                  {isCashOnDelivery ? `Order ${codOrderId}` : "Access granted"}
                </span>
              </div>
