...
import { BookOpen, Globe2, GraduationCap, UserRound } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { notFound } from "next/navigation";

import { BookResponse } from "@/app/(withoutSidebarLayout)/books/Books.type.ts";
import AuthorBooksGrid from "./AuthorBooksGrid";
...
function normalizeBooks(books: AuthorProfile["books"]): BookResponse {
...
export default async function AuthorPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const books = normalizeBooks(author.books);
    ...
    <main className='min-h-screen bg-slate-50 py-14'>
      <div className='mx-auto max-w-6xl px-4 sm:px-6 lg:px-8'>
          ...
          <div className='bg-linear-to-br from-orange-50 via-white to-blue-50 p-6 sm:p-9'>
            <div className='flex flex-col gap-6 sm:flex-row sm:items-center'>
              ...
              <div className='min-w-0 flex-1'>
                ...
                {author.bio && (
                  <p className='mt-4 max-w-3xl whitespace-pre-line text-sm leading-7 text-slate-600'>
                    {author.bio}
                  </p>
                )}
                ...
                <div className='mt-5 flex flex-wrap gap-3'>
                  {author.instructor && (
                    <Link
                      href={`/instructors/${author.instructor.userId}`}
                      className='inline-flex items-center gap-2 rounded-xl bg-[#074079] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#053052]'
                    >
                  ...
                  {author.websiteUrl && (
                    <Link
                      ...
                      className='inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:border-orange-300 hover:text-orange-600'
                    >
        ...
        <section className='mt-10'>
          ...
          {books.length ? (
            <AuthorBooksGrid books={books} />
          ) : (
            <div className='rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500'>
              No published books are available for this author yet.
            </div>
          )}
        </section>
        ...
        {author.instructor && (
          <section className='mt-12'>
            ...
            {author.courses.length ? (
                    ...
                    <div className='p-5'>
                      ...
                      <div className='mt-3 flex items-center justify-between text-sm text-slate-500'>
                        ...
                        <span className='font-black text-slate-900'>
                          ৳
                          {Number(course.discountPrice ?? course.originalPrice).toLocaleString(
                            "en-BD"
                          )}
                        </span>
                      </div>
              ...
              <div className='rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500'>
                This author is an AloSkill instructor, but has no published courses yet.
              </div>
            )}
          </section>
        )}
